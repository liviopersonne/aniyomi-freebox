...
object HttpFreeboxService {
    ...
    var state: FreeboxState = FreeboxState.DISCONNECTED
    private var freebox: Freebox? = null
    private var appToken: String? = null
    private var sessionToken: String? = null
    private var track_id: Int? = null
    ...
    // Start state: PENDING or +
    suspend fun getSessionToken(challenge: String? = null): Boolean {
        if (appToken == null) { state = FreeboxState.DISCONNECTED; return false }
        return try {
            withContext(Dispatchers.IO) {
    ...
    // Start state: CONNECTED_NO_PLAYER or CONNECTED
    suspend fun logout(): Boolean {
        if (sessionToken == null) { state = FreeboxState.DISCONNECTED; return true }
        getSessionToken()
        return try {
            withContext(Dispatchers.IO) {
                // Post request
    ...
    // Start state: CONNECTED_NO_PLAYER
    private suspend fun getReceiversAirmedia(): List<AirmediaReceiver> {
        if (sessionToken == null) { state = FreeboxState.DISCONNECTED; return emptyList() }
        getSessionToken()
        return try {
            withContext(Dispatchers.IO) {
                val request = Request.Builder()
    ...
    suspend fun playVideo(url: String): Boolean {
        getSessionToken()
        if (freeboxPlayerAvailable() != 1) { state = FreeboxState.CONNECTED_NO_PLAYER; return false }
        return try {
            withContext(Dispatchers.IO) {
